...
CREATE TABLE `post` (
  ...
  `images` TEXT COMMENT '图片列表(存储JSON字符串)',
  `videos` TEXT COMMENT '视频列表(存储JSON字符串)',
...
CREATE TABLE `work` (
  ...
  `images` TEXT NOT NULL COMMENT '作品图片列表(存储JSON字符串)',
  `cover_image` VARCHAR(500) DEFAULT NULL COMMENT '封面图',
  `category_id` BIGINT NOT NULL COMMENT '分类ID',
  `tags` TEXT COMMENT '标签列表(存储JSON字符串)',
...
CREATE TABLE `work_category` (
  ...
  KEY `idx_parent` (`parent_id`),
  KEY `idx_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='作品分类表';
...
CREATE TABLE `work_tag` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='作品标签表';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='作品标签关联表';
...
CREATE TABLE `mall_goods` (
  ...
  `cover_image` VARCHAR(500) DEFAULT NULL COMMENT '封面图',
  `images` TEXT COMMENT '商品图片列表(存储JSON字符串)',
  `category_id` BIGINT NOT NULL COMMENT '分类ID',
...
CREATE TABLE `mall_category` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_parent` (`parent_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='商城商品分类表';
...
CREATE TABLE `collect_record` (
  ...
  KEY `idx_user` (`user_id`),
  KEY `idx_target_type` (`target_id`, `target_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='收藏记录表';
...
CREATE TABLE `feedback` (
  ...
  `images` TEXT COMMENT '截图(存储JSON字符串)',
